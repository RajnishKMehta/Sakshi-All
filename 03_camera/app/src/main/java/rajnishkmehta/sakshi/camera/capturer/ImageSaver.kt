...
import rajnishkmehta.sakshi.camera.IMAGE_NAME_PREFIX
import rajnishkmehta.sakshi.camera.ITEM_TYPE_IMAGE
import rajnishkmehta.sakshi.camera.capturer.ImageSaverException.Place
import rajnishkmehta.sakshi.camera.clearExif
import rajnishkmehta.sakshi.camera.fixExif
...
) : ImageCapture.OnImageCapturedCallback()
{
    ...
    private fun getAndroidMimeType() = MimeTypeMap.getSingleton().getMimeTypeFromExtension(imageFileFormat) ?: "image/*"
    ...
    fun obtainOutputUri(): Uri? {
        if (saveToMediaStore()) {
            val cv = ContentValues().apply {
                ...
                put(MediaStore.MediaColumns.MIME_TYPE, getAndroidMimeType())
            ...
            try {
                ...
                return DocumentsContract.createDocument(contentResolver, treeDocumentUri, getAndroidMimeType(), fileName())!!
            } catch (e: Exception) {
