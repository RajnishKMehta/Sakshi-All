...
import rajnishkmehta.sakshi.camera.analyzer.QRAnalyzer
import rajnishkmehta.sakshi.camera.ktx.applyPreviewRatio
import rajnishkmehta.sakshi.camera.ui.activities.CaptureActivity
import rajnishkmehta.sakshi.camera.ui.activities.MainActivity
import rajnishkmehta.sakshi.camera.ui.activities.MoreSettings
import rajnishkmehta.sakshi.camera.ui.activities.SecureActivity
...
import rajnishkmehta.sakshi.camera.ui.activities.VideoOnlyActivity
import rajnishkmehta.sakshi.camera.ui.showIgnoringShortEdgeMode
import rajnishkmehta.sakshi.camera.util.edit
...
class CamConfig(private val mActivity: MainActivity) {
        ...
        cameraProviderFuture.addListener(fun() {
            ...
            thread {
                try {
                    ...
                    // thread.
                    DebugLogger.e(TAG, "Extensions manager initialization failed", e)
    ...
    private fun probeExtension(
        ...
        return try {
            ...
            // remember.
            DebugLogger.w(TAG, "Extension mode $extensionMode is advertised but unusable here", e)
            false
        } catch (e: Exception) {
            ...
            DebugLogger.w(TAG, "Probing extension mode $extensionMode failed, will retry later", e)
            null
        }
    }
    ...
    private fun canVerifyFeatureCombinations(): Boolean {
        ...
        val cameraInfo = try {
            ...
            DebugLogger.w(TAG, "Unable to resolve camera info for feature combination gate", exception)
            return false
        }
        ...
        val queryVersion = try {
            ...
            DebugLogger.w(TAG, "Camera info carries no camera2 characteristics", exception)
            null
        }
    ...
    private fun videoQualityAsGroupableFeature(): GroupableFeature? {
        val quality = if (videoQuality == Quality.HIGHEST) {
            val cameraInfo = try {
                ...
                DebugLogger.w(TAG, "Unable to resolve camera info for quality lookup", exception)
                return null
            }
        ...
        return when (quality) {
            ...
            Quality.SD -> GroupableFeatures.SD_RECORDING
                ...
                DebugLogger.w(TAG, "No groupable feature equivalent for video quality $quality")
                null
            }
        }
    }
    ...
    private fun onFeaturesSelected(
        ...
        DebugLogger.i(TAG, "Requested $requested but got $selected")
    ...
    fun startCamera(forced: Boolean = false) {
        ...
        if (extMode != ExtensionMode.NONE) {
            ...
            if (em != null && isExtensionUsable(cameraSelector, lensFacing, extMode)) {
                ...
                DebugLogger.e(TAG, "Mode $currentMode isn't available for this device")
            }
        }
        ...
        if (isVideoMode && snapshotUseCase != null) {
            val cameraInfo = try {
                ...
                DebugLogger.e(TAG, "Failed to query camera info", exception)
            ...
            if (dropReason != null) {
                DebugLogger.i(TAG, "$dropReason; disabling snapshots while recording")
        ...
        try {
            ...
            if (key == null) {
                ...
                if (exception is IllegalArgumentException) {
                    DebugLogger.e(TAG, "Failed to bind use cases", exception)
                    mActivity.showMessage(mActivity.getString(R.string.bind_failure))
                    return
                }
                throw exception
            ...
            DebugLogger.e(TAG, "Extension mode $extMode failed to bind; disabling it", exception)
    ...
    private fun buildTabs() {
        ...
        DebugLogger.i(TAG, "Refreshing tabs...")
